...
This study investigates the use of argon as a propellant, due to its abundance, low cost, and potential to make space travel more economically accessible. A major drawback of argon, however, is its low propellant utilization efficiency ("η" _"u" ), which can be approximated by the ratio of the ion mass flow rate ("m"  ̇_"i" ) to the total propellant mass flow rate ("m"  ̇_"A" ), or alternatively, by the relationship between the ionization region length ("L" _"i" ) and the ionization mean free path ("λ" _"i" ), as shown below:
